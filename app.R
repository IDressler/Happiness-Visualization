...
ui <- fluidPage(
  ...
  # Application title
  titlePanel("Happiness Levels From 1972 To 2021 In The United States"),
...
server <- function(input, output) {
  ...
  output$distPlot <- renderPlotly({
    ...
    fig <- fig %>% add_trace(
      ...
      type = 'scatter', 
      mode = 'lines')
    fig <- fig %>% layout(xaxis = x, yaxis = y, title = "Happiness Levels Over Time")
  })
